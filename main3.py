...
def run_detection_pipeline_bytes(image_bytes: bytes):
    ...
    Saves a temp file in the current directory, then deletes it.
    """
    ...
    # Step 3: Save image as a temp file in the current folder
    ...
    try:
        if plant_type == "LEAF":
            prediction, confidence = predict_leaf_disease(temp_filename)
        else:
            prediction, confidence = predict_fruit_disease(temp_filename)
    finally:
    ...
    return {
        ...
        "severity": severity_info["severity"],  # NEW
        "severity_score": severity_info["severity_score"],  # NEW
        "severity_message": severity_info["message"],  # NEW
        "confidence_percent": severity_info["confidence_percent"],  # NEW
